...
export class BoilerplateCard extends LitElement {
  ...
  protected render(): TemplateResult | void {
    ...
    if (!entity) {
      ...
      errorCard.innerHTML = "No Entity defined"
      }
    ...
    if (!entityState) {
      ...
      errorCard = document.createElement('div');
      errorCard.innerHTML = "No State for the sensor"
      }else{
        
      }

      // console.info(entityState.state);
    // if (entityState.state == "off") {
    //   errorCard = document.createElement('div');
    //   errorCard.innerHTML = "Kodi is off";
    // }
    ...
    return html`
      ...
      hasDoubleClick: hasAction(this.config.double_tap_action),
        ...
        tabindex="0"
        .label=${`Boilerplate ${this.config.entity || 'No Entity Definedl;l;l;'}`}
      >
      ${errorCard?
        html `${errorCard}`:
        html `${this._buildSearchForm}`
      }
  ...
  private _buildSearchForm() {
    return html`
      <div id='search-form'>
        <mwc-textfield id='form_input_search' outlined label="Search criteria"></mwc-textfield>
        <div id='form-btns'>
          <mwc-button id="form-btn-search"  label='Search' raised></mwc-button>
          <mwc-button id="form-btn-clear"  label='Clear' raised></mwc-button>
          <mwc-button id="form-btn-recent"  label='Recent' raised></mwc-button>
        </div>
